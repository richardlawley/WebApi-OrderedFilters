Properties {
	...
	$solution_name = "$base_dir\WebApi-OrderedFilters.sln"
...
Task NuGetBuild {
	& $nuget_exe pack "$base_dir/src/RichardLawley.WebApi.OrderedFilters/RichardLawley.WebApi.OrderedFilters.csproj" -Build -OutputDirectory $build_artifacts_dir -Verbose -Properties Configuration=Release
}
...
Task RunUnitTests -depends PrepareForTest, Build {
	...
	& "$nunitConsole" "$build_artifacts_dir\RichardLawley.WebApi.OrderedFilters.Tests.dll" /nologo /nodots /framework:net-4.0 "/xml=$test_result"
